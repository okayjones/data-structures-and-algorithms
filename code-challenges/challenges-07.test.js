...
const forLoopTwoToThe = (arr) => {
  let result = [];
  for (const n of arr) result.push(Math.pow(2, n));
  return result;
};
...
const forEachTwoToThe = (arr) => {
  let result = [];
  arr.forEach(n => result.push(Math.pow(2, n)));
  return result;};
...
const evenOdd = (arr) => arr.map(n => {
  return (typeof n != 'number') ? 'N/A' : (n % 2 === 0) ? 'even' : 'odd';
});
...
const extractAbilities = (arr) => arr.map(a => a.ability.name);
