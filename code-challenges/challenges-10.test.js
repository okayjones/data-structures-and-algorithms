...
const createServer = () => {
  ...
  app.use('/hello', helloHander);
  app.use('/aboutme', aboutHandler);
  app.use('/favoritefoods', foodHandler);
  app.use('*', notFoundHandler);

  function helloHander(req,res){
    const response = 'Hello World!';
    res.status(200).send(response);
  }

  function aboutHandler(req,res){
    const response = 'Hi, I\'m Logan.';
    res.status(200).send(response);
  }

  function foodHandler(req,res){
    const response = ['Pad see ew', 'anything made out of a potato', 'sushi, beets'];
    res.status(200).send(response);
  }
...
const count = (target, input) => {
  return input
    .map(arr => arr.filter(num => num === target))
...
const totalSum = (input) => {
  return input
    .map(arr => arr.reduce((total, num) => total += num), 0)
...
const divisibleByFiveTwoToThePower = (input) => {
  return input
    .map(arr => arr.filter(num => typeof num === 'number' && num % 5 === 0))
...
let findShortest = (data) => {
  return data
    .reduce((s, p) => (Number(p.height) < Number(s.height)) ? p : s)
    .name;
};
